    ...
    if ~(MRSCont.flags.didProcess == 1 && MRSCont.flags.speedUp && isfield(MRSCont, 'processed') && (kk > length(MRSCont.processed.A)))  || ~strcmp(MRSCont.ver.Osp,MRSCont.ver.CheckOsp)
        ...
        if raw.dims.subSpecs == 0
            ...
            if raw.dims.extras == 0
                ...
                dimDW = raw.dims.subSpecs;
                dimGrad = raw.dims.extras;
                nGrad = raw.sz(dimGrad);
                takeFunction = @op_takesubspec;
                needToReOrder = 1;
            end
        ...
        % The structure for 4-dimensional data is a little complicated at
        % the moment, until we have the full new framework implemented. For
        % now, we'll do a little bit of an ugly hack here that will work
        % for the workshop example data.
        ...
        if needToReOrder
            ...
            directions = [];
                ...
                for bb = 1:nGrad
                    tempRaw{bb} = op_takeextras(raw, bb);
                    
                    if bb == 1
                        tempOut = tempRaw{bb};
                    else
                        tempOut = op_concatSubspecs(tempOut, tempRaw{bb});
                    end
                ...
                for aa = 1:nDW
                    for bb = 1:nGrad
                        bVals(end+1)    = raw.nii_mrs.hdr_ext.dim_6_header.Bval(aa);
                        directions(end+1,:) = raw.nii_mrs.hdr_ext.dim_7_header.direction(bb,:);
                    end
                end
                
                % Update the header
                tempOut.nii_mrs.hdr_ext.dim_6_header.Bval = bVals;
                tempOut.nii_mrs.hdr_ext.dim_6_header.direction = directions;
                tempOut.nii_mrs.hdr_ext.dim_6_info = 'b values and directions';
                tempOut.nii_mrs.hdr_ext.dim_7 = [];
                tempOut.nii_mrs.hdr_ext.dim_7_info = [];
                tempOut.nii_mrs.hdr_ext.dim_7_header = [];
                
                MRSCont.raw{kk} = tempOut;
                raw             = MRSCont.raw{kk}; 
        ...
        if MRSCont.flags.hasRef
            ...
            directions = [];
                ...
                for bb = 1:nGrad
                    tempRaw{bb} = op_takeextras(raw_ref, bb);
                ...
                for aa = 1:nDW
                    for bb = 1:nGrad
                        bVals(end+1)    = raw_ref.nii_mrs.hdr_ext.dim_6_header.Bval(aa);
                        directions(end+1,:) = raw_ref.nii_mrs.hdr_ext.dim_7_header.direction(bb,:);
                    end
                end
                ...
                MRSCont.raw_ref{kk} = tempOut;
                raw_ref             = MRSCont.raw_ref{kk};
        ...
        for dd = 1:nDW
            ...
            if MRSCont.flags.hasRef
                ...
                if raw_ref_temp.averages > 1 && raw_ref_temp.flags.averaged == 0
                    % Pre-phase water averages by dividing by the first
                    % point
                    raw_ref_temp = op_phaseFirstPoint(raw_ref_temp);
                ...
                MRSCont.processed.ref{kk}{dd}     = raw_ref_temp;
        ...
        for ss = 1 : length(SubSpec)         
            for dd = 1:nDW
                ...
                if ~(strcmp(SubSpec{ss},'ref') || strcmp(SubSpec{ss},'w') || strcmp(SubSpec{ss},'mm'))
                    ...
                    MRSCont.QM.freqShift.(SubSpec{ss})(kk,dd) = 0;
