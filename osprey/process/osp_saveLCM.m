...
for kk = 1:MRSCont.nDatasets
    ...
    if MRSCont.flags.isUnEdited
        ...
        MRSCont.opts.fit.lcmodel.outfileA{kk}{1} = outfile;
    ...
    elseif MRSCont.flags.isDWMRS
        ...
        % Get the b values
        if isfield(MRSCont.raw{kk}.nii_mrs.hdr_ext.dim_6_header, 'Bval')
            bVals = MRSCont.raw{kk}.nii_mrs.hdr_ext.dim_6_header.Bval;
        end
        if isfield(MRSCont.raw{kk}.nii_mrs.hdr_ext.dim_6_header, 'direction')
            directions = MRSCont.raw{kk}.nii_mrs.hdr_ext.dim_6_header.direction;
        end
        ...
        for dd = 1:nDW
            ...
            % Create an individual string
            if exist('bVals', 'var')
                if exist('directions', 'var')
                    uniqueFileID = ['_bVal' num2str(bVals(dd)) '_dir' num2str(dd)];
                else
                    uniqueFileID = ['_bVal' num2str(bVals(dd))];
                end
            end
            
            outfile         = fullfile(saveDestination,'metabs', [name uniqueFileID '_LCM_A.RAW']);
            ...
            if MRSCont.flags.hasRef
                ...
                if strcmpi(MRSCont.vendor, 'GE') || strcmp(MRSCont.datatype,'DATA')
                    te_ref                      = MRSCont.processed.A{kk}{dd}.te;
                    [path_ref,filename_ref,~]   = fileparts(MRSCont.files{kk});
                else
                    te_ref                      = MRSCont.processed.ref{kk}{dd}.te;
                ...
                % Set up complete output filename strings, then write LCM .RAW files.
                outfileRef      = fullfile(saveDestination,'ref', [name_ref uniqueFileID '_LCM_REF.RAW']);
                RF              = io_writelcm(MRSCont.processed.ref{kk}{dd},outfileRef,te_ref);
                
                MRSCont.opts.fit.lcmodel.outfileRef{kk}{dd}     = outfileRef;
