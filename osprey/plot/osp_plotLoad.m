...
elseif (isfield(MRSCont.flags,'isDWMRS') && (MRSCont.flags.isDWMRS == 1))
        if ~exist('VoxelIndex') && (MRSCont.flags.isDWMRS == 1)
            VoxelIndex = 1;
        end
        ...
        if MRSCont.raw{kk}.dims.subSpecs == 0
            if MRSCont.raw{kk}.dims.extras == 0
                error('This dataset does not appear to have sub-spectra. ABORT!');
            else
                dimDW = MRSCont.raw{kk}.dims.extras;
                takeFunction = @op_takeextras;
            end
        else
            dimDW = MRSCont.raw{kk}.dims.subSpecs;
            takeFunction = @op_takesubspec;
        end
        
    switch which
        case 'mets'
            dataToPlot = takeFunction(MRSCont.raw{kk}, VoxelIndex);
            dataToPlot = op_freqrange(dataToPlot, ppmmin, ppmmax);
        case 'mm' %re_mm
            dataToPlot = takeFunction(MRSCont.raw_mm{kk}, VoxelIndex);
            dataToPlot = op_freqrange(dataToPlot, ppmmin, ppmmax);   %re_mm
        case 'ref'
            dataToPlot = takeFunction(MRSCont.raw_ref{kk}, VoxelIndex);
            dataToPlot = op_freqrange(dataToPlot, ppmmin, ppmmax);
        case 'w'
            dataToPlot = takeFunction(MRSCont.raw_w{kk}, VoxelIndex);
            dataToPlot = op_freqrange(dataToPlot, ppmmin, ppmmax);
        otherwise
            error('Input for variable ''which'' not recognized. Needs to be ''mets'' (metabolite data), ''ref'' (reference data), or ''w'' (short-TE water data).');
    end
    ...
    if isempty(ylab)
        ...
        if ax == 1
            if MRSCont.flags.isDWMRS
                title(axesHandles.(axesNames{ax}),[figTitle 'Subspectra ' num2str(VoxelIndex) ' - b = ' num2str(dataToPlot.nii_mrs.hdr_ext.dim_6_header.Bval(VoxelIndex)) ], 'Interpreter', 'none', 'Color', MRSCont.colormap.Foreground);
            else
