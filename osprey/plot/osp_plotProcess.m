...
%% out = osp_plotProcess(MRSCont, kk, which, VoxelIndex, ppmmin, ppmmax)
...
elseif (isfield(MRSCont.flags,'isDWMRS') && (MRSCont.flags.isDWMRS == 1))
    
    switch which_spec
        case {'A', 'B', 'C', 'D'}
            raw            = MRSCont.raw{kk};
            procDataToPlot = MRSCont.processed.(which_spec){kk}{VoxelIndex};
            ...
            elseif MRSCont.flags.isUnEdited || MRSCont.flags.isDWMRS
                    raw_A = raw;                                        % Get all averages
            end
            ...
            if raw_A.dims.subSpecs == 0
                if raw_A.dims.extras == 0
                    error('This dataset does not appear to have sub-spectra. ABORT!');
                else
                    dimDW = raw_A.dims.extras;
                    takeFunction = @op_takeextras;
                end
            else
                dimDW = raw_A.dims.subSpecs;
                takeFunction = @op_takesubspec;
            end
        
            eval(['rawDataToPlot = takeFunction(raw_' which_spec ', ' num2str(VoxelIndex) ');']);
            rawDataToScale = raw_A;                                     % This is used to get consistent yLims
...
elseif (isfield(MRSCont.flags,'isDWMRS') && (MRSCont.flags.isDWMRS == 1))
    if isfield(MRSCont.QM.freqShift, which_spec)
        switch which_spec
            case {'A', 'B', 'C', 'D'} 
                refShift = -repmat(MRSCont.QM.freqShift.(which_spec)(kk, VoxelIndex), size(fs));
                fs = fs - refShift;
                for jj = 1:size(applyDataToScale.fids,2)
                    applyDataToScale.fids(:,jj) = applyDataToScale.fids(:,jj) .* ...
                        exp(1i*fs(jj)*2*pi*t') * exp(1i*pi/180*phs(jj));
            ...
            case {'diff1', 'diff2','diff3', 'sum'}
                refShift = -repmat(MRSCont.QM{VoxelIndex}.freqShift.(which_spec)(kk, VoxelIndex), size(fs{1}));
                for ss = 1 : length(fs)
                    fs{ss} = fs{ss} - refShift;
                    for jj = 1:size(applyDataToScale.fids,2)
                        applyDataToScale.fids(:,jj,ss) = applyDataToScale.fids(:,jj,ss) .* ...
    ...
    case {'A', 'B', 'C', 'D'} 
        ...
        elseif (isfield(MRSCont.flags,'isMRSI') && (MRSCont.flags.isMRSI == 1))
            ...
            if MRSCont.flags.isDWMRS
                weights = MRSCont.processed.(which_spec){kk}{VoxelIndex}.specReg.weights;
            else
...
elseif (isfield(MRSCont.flags,'isDWMRS') && (MRSCont.flags.isDWMRS == 1))
    if isfield(MRSCont.QM.drift.pre, which_spec)
        if length(MRSCont.QM.drift.pre.(which_spec){kk}{VoxelIndex}) > 1
            crDriftPre = MRSCont.QM.drift.pre.(which_spec){kk}{VoxelIndex} + MRSCont.QM.freqShift.(which_spec)(kk,VoxelIndex)/applyDataToPlot.txfrq*1e6;
            crDriftPost = MRSCont.QM.drift.post.(which_spec){kk}{VoxelIndex} + MRSCont.QM.freqShift.(which_spec)(kk,VoxelIndex)/applyDataToPlot.txfrq*1e6;
            hold(ax_drift, 'on');
            colors = ones(length(crDriftPre),1).*colormap.Foreground;
            for dots = 1 : length(crDriftPre)
                colors(dots,1) = colors(dots,1) + (1 - colors(dots,1)) * (1-weights(dots));
            ...
            scatter(ax_drift, [1:length(crDriftPre)],crDriftPre'-(MRSCont.QM.freqShift.(which_spec)(kk,VoxelIndex)/procDataToPlot.txfrq*1e6),36,ones(length(crDriftPre),1).*colormap.LightAccent);
            scatter(ax_drift, [1:length(crDriftPost)],crDriftPost'-(MRSCont.QM.freqShift.(which_spec)(kk,VoxelIndex)/procDataToPlot.txfrq*1e6),36,colors,'filled','MarkerEdgeColor',colormap.Foreground);    

            text(ax_drift, length(crDriftPre)*1.05, crDriftPre(end)-(MRSCont.QM.freqShift.(which_spec)(kk,VoxelIndex)/procDataToPlot.txfrq*1e6), 'Pre', 'Color', colormap.LightAccent);
            text(ax_drift, length(crDriftPost)*1.05, crDriftPost(end)-(MRSCont.QM.freqShift.(which_spec)(kk,VoxelIndex)/procDataToPlot.txfrq*1e6), 'Post', 'Color', colormap.Foreground);
            set(ax_drift, 'YLim', [3.028-0.1 3.028+0.1]);
            yticks([3.028-0.08 3.028-0.04 3.028 3.028+0.04 3.028+0.08]);
            yticklabels({'2.94' '2.98' '3.02' '3.06' '3.10'});
            x = xlim;
