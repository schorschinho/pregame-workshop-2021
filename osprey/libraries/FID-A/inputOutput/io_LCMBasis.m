...
disp('We are going to rename your basis set according to the Osprey naming convention now.')
Osp_names = {'Ala','Asc','Asp', 'bHB','bHG', 'Cit', 'Cr', 'CrCH2', 'EtOH', 'GABA',...
             ...
             'MM09', 'MM12', 'MM14', 'MM17', 'MM20', 'Lip09', 'Lip13', 'Lip20', 'MMexp'};
         
         
filenames = fieldnames(Read);
for mm = 1 : length(fieldnames(Read))
    % Check if the incoming metabolite name is in the list
    incomingName = filenames{mm};
    
    indexMatch = find(strcmpi(Osp_names,incomingName),1);
    if ~isempty(indexMatch)
        fprintf('Found %s in your basis set and matched it to Osprey entry %s. \n', incomingName, Osp_names{indexMatch});
        cRead.(incomingName) = Read.(filenames{mm});
        Osp_names{indexMatch} = '';
    else
        
        disp(['The name in your basis set is: ' filenames{mm}]);
        prompt = ['Type the name of the matching metabolite from the list or press return if no matching metabolite is in the list: \n' sprintf('%8s %8s %8s %8s %8s \n', Osp_names{:}) '\n'];
        name = input(prompt,'s');
        if ~isempty(name)
            ind = find(strcmpi(Osp_names,name));
            if ~isempty(ind)
                cRead.(name) = Read.(filenames{mm});
                fprintf('Found %s in your basis set and matched it to Osprey entry %s. \n', incomingName, Osp_names{ind});
                Osp_names{ind} = '';
            else
                warning('Did not find your input (%s) in the list. Skipping!', name);
            end
        end
    end
end
